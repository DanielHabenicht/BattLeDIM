...
try:
    with open(os.path.join(os.getcwd(), "dataset_configuration_historical.yalm"), 'r') as f:
...
class LeakDatasetCreator:
    def __init__(self):
        ...
        self.time_stamp = pd.date_range(
            start_time, end_time, freq=str(self.time_step / 60) + "min")

        # Simulation duration in steps
        self.wn.options.time.duration = (
            len(self.time_stamp) - 1) * 300  # 5min step
        self.TIMESTEPS = int(self.wn.options.time.duration /
    ...
    def dataset_generator(self):
        ...
        for leak_i in range(0, number_of_leaks):
            ...
            self.wn = wntr.morph.split_pipe(
                self.wn, pipe_id, f'{pipe_id}_Bleak', node_leak)
            leak_node[leak_i] = self.wn.get_node(
                self.wn.node_name_list[self.wn.node_name_list.index(node_leak)])
            ...
            if 'incipient' in leak_type[leak_i]:
                ...
                increment_leak_diameter = np.arange(
                    increment_leak_diameter, leak_diameter[leak_i], increment_leak_diameter)
                increment_leak_area = 0.75 * \
                    sqrt(2 / 1000) * 990.27 * 3.14159 * \
                    (increment_leak_diameter/2)**2
                leak_magnitude = 0.75 * \
                    sqrt(2 / 1000) * 990.27 * leak_area[leak_i]
                pattern_array = [0] * (ST) + increment_leak_area.tolist() + \
                    [leak_magnitude] * (ET - PT + 1) + \
                    [0] * (self.TIMESTEPS - ET)
                ...
                leak_starts[leak_i] = leak_starts[leak_i]._date_repr + \
                    ' ' + leak_starts[leak_i]._time_repr
                leak_ends[leak_i] = self.time_stamp[ET - 1]
                leak_ends[leak_i] = leak_ends[leak_i]._date_repr + \
                    ' ' + leak_ends[leak_i]._time_repr
                leak_peak_time[leak_i] = self.time_stamp[PT -
                                                         1]._date_repr + ' ' + self.time_stamp[PT - 1]._time_repr
                ...
                leak_node[leak_i]._leak_start_control_name = str(
                    leak_i) + 'start'
        ...
        if results:
            ...
            for leak_i in range(0, number_of_leaks):
                ...
                leaks = results.node[leak_param[leak_i]
                                     ][str(leak_node[leak_i])].values
                ...
                df1.to_csv(os.path.join(leakages_folder,
                           f"Leak_{NODEID}_Info.csv"))
                df2.to_csv(os.path.join(leakages_folder,
                           f"Leak_{NODEID}_Demand.csv"))
                writer = pd.ExcelWriter(os.path.join(
                    leakages_folder, f'Leak_{NODEID}.xlsx'), engine='xlsxwriter')
            ...
            for j in range(0, self.wn.num_nodes):
                ...
                if node_id in amrs:
                    ...
                    dem = [round(elem, decimal_size)
                           for elem in dem]  # CMH / L/s
                ...
                if node_id in level_sensors:
                    ...
                    level_pres = [round(elem, decimal_size)
                                  for elem in level_pres]
            ...
            writer = pd.ExcelWriter(os.path.join(
                results_folder, "Measurements.xlsx"), engine='xlsxwriter')
            ...
            df1.to_csv(os.path.join(results_folder,
                       "Pressures.csv"), index=False)
            df2.to_csv(os.path.join(results_folder,
                       "Demands.csv"), index=False)
            df3.to_csv(os.path.join(results_folder, "Flows.csv"), index=False)
            df4.to_csv(os.path.join(results_folder, "Levels.csv"), index=False)
